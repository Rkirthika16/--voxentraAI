...
# Ordered definition of the 10 required complaint details
FIELD_KEYS = [
    "problem_description",
    "exact_location",
    "street_road_name",
...
FIELD_METADATA = {
    "problem_description": {
        ...
        "question_en": "Could you please describe the civic problem in detail? (e.g. water leakage, power cut, uncollected garbage, road damage)",
        ...
        "spoken_en": "Please describe what civic problem has occurred.",
        "spoken_ta": "உங்களுக்கு ஏற்பட்டுள்ள பொது பிரச்சனை என்ன என்று கூறவும்.",
        "spoken_tanglish": "Ungaloda problem enna nu sollunga."
    },
    "exact_location": {
        "label_en": "Exact Location",
        "label_ta": "சரியான இடம்",
        "label_tanglish": "Exact Location",
        "question_en": "Where exactly did this problem occur? (e.g. Near Bus Stand, opposite Government Hospital, Ward 12)",
        "question_ta": "இந்தப் பிரச்சனை சரியாக எந்த இடத்தில் ஏற்பட்டுள்ளது? (எ.கா: பேருந்து நிலையம் அருகில், அரசு மருத்துவமனை எதிரில், வார்டு 12)",
        "question_tanglish": "Indha problem exact-ah endha edathula nadanthuchu? (e.g. Bus stand kitta, Hospital opposite, Ward 12)",
        "spoken_en": "Where exactly did this problem occur?",
        "spoken_ta": "இந்தப் பிரச்சனை சரியாக எந்த இடத்தில் ஏற்பட்டுள்ளது?",
        "spoken_tanglish": "Indha problem exact-ah endha location-la nadanthuchu?"
    },
    "street_road_name": {
        "label_en": "Street / Road Name",
        "label_ta": "தெரு / சாலை பெயர்",
        "label_tanglish": "Street / Road Name",
        "question_en": "Which street or road is affected? (e.g. Cross Cut Road, Gandhi Street, Main Bazaar, 2nd Avenue)",
        "question_ta": "பாதிக்கப்பட்ட தெரு அல்லது சாலையின் பெயர் என்ன? (எ.கா: காந்தி தெரு, மெயின் ரோடு, கிராஸ் கட் ரோடு)",
        "question_tanglish": "Endha street or road affected aagi irukku? (e.g. Gandhi Street, Main Road, Cross Cut Road)",
    ...
    "district_area": {
        ...
        "question_en": "In which District and Area/Town did this occur? (e.g. Coimbatore - Gandhipuram, Chennai - Anna Nagar, Madurai - Melur)",
        "question_ta": "இது எந்த மாவட்டம் மற்றும் பகுதியில் நிகழ்ந்துள்ளது? (எ.கா: கோயம்புத்தூர் - காந்திபுரம், சென்னை - அண்ணா நகர், மதுரை - மேலூர்)",
        "question_tanglish": "Idhu endha District and Area-la nadanthuchu? (e.g. Coimbatore - Gandhipuram, Chennai - Anna Nagar)",
        "spoken_en": "In which district and area did this occur?",
        "spoken_ta": "இது எந்த மாவட்டம் மற்றும் பகுதியில் நிகழ்ந்துள்ளது?",
        "spoken_tanglish": "Idhu endha District and Area-la nadanthuchu?"
    },
    "landmark": {
        "label_en": "Landmark",
        "label_ta": "அடையாளம் / முக்கிய இடம்",
        "label_tanglish": "Landmark",
        "question_en": "What nearby landmark can help our inspection team identify the spot? (e.g. Near Murugan Temple, Behind Indian Bank ATM, Opposite High School)",
        "question_ta": "எங்கள் ஆய்வு குழுவினர் இடத்தை அடையாளம் காண அருகிலுள்ள முக்கிய அடையாளம் (Landmark) என்ன? (எ.கா: முருகன் கோவில் அருகில், இந்தியன் வங்கி எதிரில்)",
    ...
    "current_status": {
        ...
        "question_en": "Is the problem still happening right now, or has it been partially/temporarily resolved? (e.g. Still active and severe, Still leaking, Temporarily blocked)",
        ...
        "question_tanglish": "Ippo current status enna? Still problem irukka illa temporary-ah stop aacha? (e.g. Still happening, Ongoing, Not fixed yet)",
    ...
    "additional_details": {
        ...
        "label_ta": "கூடுதல் விவரங்கள்",
        "label_tanglish": "Additional Details",
...
class ComplaintCollector:
    ...
    def extract_slots(self, text: str, current_field: Optional[str] = None) -> Dict[str, Any]:
        ...
        if current_field:
            if current_field == "citizen_details":
                ...
                if any(w in lowered for w in ["none", "no", "nothing", "illa", "illai", "இல்லை", "no additional", "nil", "nothing else"]):
                    extracted["additional_details"] = "No additional details (None specified)"
                else:
        ...
        # Location extraction from location_service
        ...
        if loc_name and loc_name != "Tamil Nadu":
            ...
            if not extracted.get("exact_location"):
                extracted["exact_location"] = loc_name

        # Problem description check if classification detects civic grievance
        category, dept, cat_conf = classify_complaint(normalized)
        if category != "Other" or any(w in lowered for w in ["leak", "broken", "cut", "garbage", "drainage", "pothole", "light", "குடிநீர்", "மின்வெட்டு", "குப்பை", "சாலை"]):
    ...
    def generate_summary(self, session: Dict[str, Any], lang: str) -> Tuple[str, str]:
        ...
        along with the spoken text for audio synthesis.
        """
        f = session["fields"]
        ...
        if lang == "Tamil":
            reply = (
                ...
                f"2. 📍 **சரியான இடம் (Exact Location):** {f.get('exact_location', 'குறிப்பிடப்படவில்லை')}\n"
                f"3. 🛣️ **தெரு / சாலை பெயர் (Street/Road):** {f.get('street_road_name', 'குறிப்பிடப்படவில்லை')}\n"
                f"4. 🏙️ **மாவட்டம் / பகுதி (District/Area):** {f.get('district_area', 'குறிப்பிடப்படவில்லை')}\n"
                f"5. 🏛️ **அடையாளம் (Landmark):** {f.get('landmark', 'குறிப்பிடப்படவில்லை')}\n"
                ...
                "உடனடியாக பதிவு செய்து துறைக்கு அனுப்ப **'ஆமாம் / பதிவு செய்க' (Confirm)** என்று கூறவும் அல்லது பொத்தானை அழுத்தவும்."
            )
            spoken = (
                f"உங்கள் புகார் விவரங்கள் அனைத்தும் சேகரிக்கப்பட்டுள்ளன. துறை {dept}, முன்னுரிமை {priority.value}. "
                "இப்புகாரை உடனடியாக பதிவு செய்து அனுப்பலாமா? ஆமாம் அல்லது சரி என்று கூறவும்."
            )
        elif lang == "Tanglish":
            reply = (
                "### 📋 Grievance Summary (10 Required Details Collected):\n\n"
                f"1. 📝 **Problem Description:** {f.get('problem_description', 'Not Specified')}\n"
                f"2. 📍 **Exact Location:** {f.get('exact_location', 'Not Specified')}\n"
                f"3. 🛣️ **Street / Road Name:** {f.get('street_road_name', 'Not Specified')}\n"
                f"4. 🏙️ **District / Area:** {f.get('district_area', 'Not Specified')}\n"
                f"5. 🏛️ **Landmark:** {f.get('landmark', 'Not Specified')}\n"
                ...
                f"9. 📌 **Additional Details:** {f.get('additional_details', 'None')}\n"
                f"10. 👤 **Citizen Details:** {f.get('citizen_details', 'Not Specified')}\n\n"
                f"🏢 **Assigned Department:** `{dept}`\n"
                f"⚡ **Priority Level:** **`{priority.value}`**\n\n"
                "**Ellam correct ah irukka? Shall I register and submit this complaint now?**\n"
            ...
            spoken = (
                ...
                "Shall I confirm and submit this complaint now?"
            )
        else:
            reply = (
                ...
                "**Please confirm if all details above are correct.**\n"
            ...
            spoken = (
                ...
                "Please confirm: shall I register this complaint now?"
            )
